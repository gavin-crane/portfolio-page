...
export default function IconSolarSystem({icons, identifier}) {
   
    const isMounted = useRef(false);
    useEffect(()=> {
    if (isMounted.current) {

        const baseOrbitalSize = 10;
        const baseOrbitSpeed = 14;
        const baseDistBetweenOrbits = 120;
        const baseSubOrbitalSize = 12;
      
        // will contains all the moons and their orbits
        let container = document.querySelector(`.system-container${identifier}`);
        ...
        function addMoonOrbit(orbitSize, moonSize, icon, subMoons = false) {
          ...
          orbit.classList.add(`orbit${identifier}`);
          ...
          moon.classList.add(`moon${identifier}`);
        ...
        let orbits = document.getElementsByClassName(`orbit${identifier}`);
        ...
        for (let i = 0; i < orbits.length; i++) {
          ...
          let moon = orbits[i].getElementsByClassName(`moon${identifier}`)[0];
    ...
    return (
        <>
            <div className ={`system-container${identifier}`}>
            </div>
        </>
    )
}
